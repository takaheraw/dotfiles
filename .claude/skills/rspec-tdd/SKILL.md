---
name: rspec-tdd
description: Rails プロジェクトで新機能の作成、バグ修正、リファクタリング時に使用。RSpec によるユニット・統合テストを中心に TDD (RED → GREEN → REFACTOR) を強制し、80%以上のカバレッジを確保。
---
...
#### E2Eテスト (Playwright推奨 / システムスペック)
- 重要なユーザーフロー
- ブラウザ自動化とUIインタラクション
- ※ E2Eテストのアーキテクチャや設計の詳細については `e2e-testing` スキルを、手動でのツール操作や検証には `playwright-cli` スキルを活用してください。
...
### E2Eテスト（Playwright / システムスペック）

E2Eテストは、より安定して高速に実行・デバッグが可能な **Playwright** の利用を推奨しています。（プロジェクトの構成により Capybara を利用している場合もあります）

Playwright を用いたE2Eテストの Page Object Model (POM) の実装方針、テストパターン、CI環境でのFlakyテスト対策などのベストプラクティスについては、**専用の `e2e-testing` スキル** に定義されています。

- テストの具体的な書き方・設計が必要な場合は `e2e-testing` スキルを参照してください。
- ブラウザを立ち上げてUIのセレクタ調査や操作ステップの自動生成を行う場合は `playwright-cli` スキルを使用してください。
