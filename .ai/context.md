...
Shared project context is maintained in `.ai/` as the single source of truth:
- `.ai/context.md` — This file (project overview, architecture, commands)
- `.ai/rules/` — Coding rules and workflow guidelines
- `.ai/skills/` — Reusable development patterns and skill definitions

Each AI agent's config symlinks to `.ai/`:
- `CLAUDE.md` → `.ai/context.md`
- `AGENTS.md` → `.ai/context.md` (for Devin, Codex, etc.)
- `.claude/rules/*` → `.ai/rules/*`
- `.claude/skills/*` → `.ai/skills/*`
- `.github/copilot-instructions.md` → `.ai/context.md`

Agent-specific settings (permissions, hooks, plugins, MCP servers) remain in each agent's own config directory (e.g. `.claude/settings.json`).
...
- **Adding a shared AI rule/skill**: Add to `.ai/rules/` or `.ai/skills/`, then create symlinks in each agent's config directory
